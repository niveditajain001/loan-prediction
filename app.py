...
@st.cache_resource
def load_artifacts():
    # Check current directory and nested directory
    possible_paths = [
        ("src/best_model.pkl", "src/scaler.pkl", "src/encoders.pkl"),
        ("Loan_Prediction_System/src/best_model.pkl", "Loan_Prediction_System/src/scaler.pkl", "Loan_Prediction_System/src/encoders.pkl")
    ]
    
    for model_p, scaler_p, encoder_p in possible_paths:
        if os.path.exists(model_p) and os.path.exists(scaler_p) and os.path.exists(encoder_p):
            return joblib.load(model_p), joblib.load(scaler_p), joblib.load(encoder_p)
